...
type PlayerQuery struct {
	...
	withRoom        *RoomQuery
	withPlayerRoles *PlayerRoleQuery
	withFKs         bool
...
func (pq *PlayerQuery) QueryRoom() *RoomQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, player.RoomTable, player.RoomColumn),
		)
...
// QueryPlayerRoles chains the current query on the "player_roles" edge.
func (pq *PlayerQuery) QueryPlayerRoles() *PlayerRoleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, player.PlayerRolesTable, player.PlayerRolesPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *PlayerQuery) Clone() *PlayerQuery {
	...
	return &PlayerQuery{
		...
		withPlayerRoles: pq.withPlayerRoles.Clone(),
...
func (pq *PlayerQuery) WithRoom(opts ...func(*RoomQuery)) *PlayerQuery {
	query := (&RoomClient{config: pq.config}).Query()
...
// WithPlayerRoles tells the query-builder to eager-load the nodes that are connected to
// the "player_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PlayerQuery) WithPlayerRoles(opts ...func(*PlayerRoleQuery)) *PlayerQuery {
	query := (&PlayerRoleClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPlayerRoles = query
	return pq
}
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	...
	if query := pq.withRoom; query != nil {
		if err := pq.loadRoom(ctx, query, nodes, nil,
			func(n *Player, e *Room) { n.Edges.Room = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withPlayerRoles; query != nil {
		if err := pq.loadPlayerRoles(ctx, query, nodes,
			func(n *Player) { n.Edges.PlayerRoles = []*PlayerRole{} },
			func(n *Player, e *PlayerRole) { n.Edges.PlayerRoles = append(n.Edges.PlayerRoles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadRoom(ctx context.Context, query *RoomQuery, nodes []*Player, init func(*Player), assign func(*Player, *Room)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Player)
	for i := range nodes {
		if nodes[i].room_players == nil {
			continue
		}
		fk := *nodes[i].room_players
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "room_players" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PlayerQuery) loadPlayerRoles(ctx context.Context, query *PlayerRoleQuery, nodes []*Player, init func(*Player), assign func(*Player, *PlayerRole)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(player.PlayerRolesTable)
		s.Join(joinT).On(s.C(playerrole.FieldID), joinT.C(player.PlayerRolesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(player.PlayerRolesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(player.PlayerRolesPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "player_roles" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
